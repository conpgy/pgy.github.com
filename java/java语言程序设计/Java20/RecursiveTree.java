...
public class RecursiveTree extends JApplet
{
	...
	class TreePanel extends JPanel
	{
		...
		protected void paintComponent(Graphics g)
		{
			...
			paintBranch(g, depth, getWidth() / 2, getHeight() - 10, getHeight() / 3, Math.PI / 2);
		}

		double angleFactor = Math.PI / 5;
		double sizeFactor = 0.58;

		public void paintBranch(Graphics g, int depth, int x1, int y1, double length, double angle)
		{
			if(depth >= 0)
			{
				...
				paintBranch(g, depth - 1, x2, y2, length * sizeFactor, angle + angleFactor);
	...
	{
		JApplet applet = new RecursiveTree();
		applet.init();
		applet.start();
		JFrame frame = new JFrame();
		frame.setDefaultCloseOperation(JFrame.EXIT_ON_CLOSE);
		frame.setTitle("RecursiveTree");
